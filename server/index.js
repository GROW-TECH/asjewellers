...
app.post('/api/admin/approve-withdrawal', async (req, res) => {
  try {
    ...
    if (reqRow.status === 'completed') {
      return res.json({ success: true, message: 'Withdrawal already completed' });
    }

    if (reqRow.status === 'failed') {
      return res.status(400).json({ success: false, message: 'Withdrawal already failed' });
    }

    // Only pending / approved can proceed
    if (!['pending', 'approved'].includes(reqRow.status)) {
      return res.status(400).json({ success: false, message: 'Invalid withdrawal state' });
    }

    const amount = Number(reqRow.amount);

    // 2️⃣ Lock request → processing
    await supabaseAdmin
      .from('withdrawal_requests')
      .update({
        status: 'processing',
    ...
    // 3️⃣ Fetch admin balance
    const { data: adminBal } = await supabaseAdmin
      .from('admin_balance')
      .select('*')
      .limit(1)
      .maybeSingle();

    if (!adminBal || Number(adminBal.balance) < amount) {
      await supabaseAdmin.from('withdrawal_requests').update({
        status: 'failed',
        payout_error: 'Insufficient admin balance',
        updated_at: new Date().toISOString(),
      }).eq('id', request_id);
    ...
    // 4️⃣ RazorpayX payout
    let payout;
    try {
      payout = await razorpayX.payouts.create({
        account_number: '778701500173',
        fund_account: {
      ...
      await supabaseAdmin.from('withdrawal_requests').update({
        status: 'failed',
        payout_error: err?.error?.description || err.message,
        updated_at: new Date().toISOString(),
      }).eq('id', request_id);
    ...
    // 5️⃣ Deduct admin balance
    await supabaseAdmin
      .from('admin_balance')
      .update({ balance: Number(adminBal.balance) - amount })
      .eq('id', adminBal.id);
    ...
    await supabaseAdmin.from('wallets').update({
      referral_balance: Number(wallet.referral_balance) - amount,
      total_balance: Number(wallet.total_balance) - amount,
...
app.post('/api/send-otp', async (req, res) => {
  try {
    ...
    console.log(`OTP for ${phone}: ${otp}`); // For testing - remove in production
    ...
    //   body: `Your AS Jewellers OTP is: ${otp}. Valid for 5 minutes.`,
    ...
    //   route: 'otp',
    //   sender_id: 'TXTIND',
    //   message: `Your AS Jewellers OTP is ${otp}`,
