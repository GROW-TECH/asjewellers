...
export default function ActivePlansScreen() {
  ...
  // Fetch active plans + attach payments for gold calculation
    ...
    try {
      ...
      const plansWithGold = await Promise.all(
        plans.map(async (plan: any) => {
          ...
          const goldRate = payments.length ? Number(payments[0].gold_rate ?? 0) : 0; // optional: use last payment's rate
  ...
  const loadUserSession = async () => {
    try {
      ...
      if (!sessionInfo?.data?.session) return router.push('/auth/login');

      const token = sessionInfo.data.session.access_token;
      if (!token) return router.push('/auth/login');

      const res = await axios.get(`${API_BASE}/api/session`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const onViewPlan = (item: any) => { if (item?.id) router.push(`/subscription/${item.id}`); };
        ...
        <View style={styles.leftHeader}>
          ...
          <View style={{ marginLeft: 12, flex: 1 }}>
            ...
            {/* <Text style={{ color: '#FFD700', fontWeight: '700', marginTop: 2 }}>
              Total Value Saved: {formatRate(totalValue)}
            </Text> */}
          </View>
        </View>
  ...
  const PlanCard = ({ item }: { item: any }) => {
    ...
    const goldValue = goldSaved * Number(item.gold_rate ?? 0);
  ...
  if (authLoading || loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ color: '#aaa', marginTop: 10 }}>{authLoading ? 'Restoring session…' : 'Loading plans…'}</Text>
      </View>
    );
  }
